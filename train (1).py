...
parser = argparse.ArgumentParser(description='This is a model training program for a dateset of flowers using pytorch',
        ...
        python train.py data_dir --arch "vgg16" (choose architecture from vgg13, vgg16 and vgg19)
...
parser.add_argument('--learning_rate', action="store", default="0.001", type=float, help="Set Learning rate",
                    dest="learning_rate")
parser.add_argument('--hidden_units', action="store", nargs=3, default=[25088, 256, 128], type=int, help="enter 3 integers between 25088 and 102 in decreasing order",
                    dest="hidden_units")
...
if 102 <= arg_hidden_units[2] <= arg_hidden_units[1] <= arg_hidden_units[0] <= 25088:
    ...
    arg_hidden_units.extend([25088, 256, 128])
    for i in range(3):
...
# freeze feature parameters, train classifier parameters
